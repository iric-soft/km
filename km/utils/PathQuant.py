...
class Path:
    def __init__(self, db_f, ref_name, variant_name, ratio, expression, min_coverage, sequence,
                 ref_ratio, ref_expression, ref_sequence, note):
        self.db_name = db_f
    ...
    def __str__(self):
        return "%s\t%s\t%s\t%.3f\t%.1f\t%d\t%s\t%.3f\t%.1f\t%s\t%s" % (self.db_name, self.ref_name,
                                                                       self.variant_name,
                                                                       self.ratio,
                                                                       self.expression,
                                                                       self.min_coverage,
...
class PathQuant:
    ...
    def refine_coef(self):
        ...
        while last_max_grad > 0.01:  ## convergence threshold
            ...
            num_iter += 1
            log.debug("Iteration = %d, max_gradient = %f", num_iter, last_max_grad)
    ...
    def get_paths(self, db_f, ref_name, name_f, seq_f, ref_path, info="", get_min_f=lambda path: 0):
        paths = []
        ref_i = -1
        for i in range(self.nb_seq):
